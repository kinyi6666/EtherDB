...
int main(int argc, char* argv[]) {
    ...
    // 3. 预绑定列绑定批量插入
    // =====================================================================
    printf("\n=== 3. Pre-bound (column binding) Batch Insert Performance Test ===\n");

    EtDBStmt* stmt = client.createStmt();
    if (!stmt) { printf("  [FATAL] createStmt failed\n"); return 1; }

    bool ok = stmt->prepare("INSERT INTO " + table + " VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    if (!ok) { printf("  [FATAL] prepare failed\n"); delete stmt; return 1; }
    printf("  [OK] Prepared: INSERT INTO %s VALUES(...10 params...)\n", table.c_str());
