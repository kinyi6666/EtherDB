...
| 流式查询拉取（`SELECT ts,col_int … WHERE col_int>5`） | **1002 万行**匹配结果 | 11.26 秒 → 约 **89 万行/秒** | **约 2.06 秒** → 约 **486 万行/秒** | 2.12 秒 → 约 474 万行/秒 |
...
- **全量取数（fetch 全部行）：两代 TDengine 都赢。** 1002 万行两列，3.4.2.8 约 2.06 秒、2.1.7.2 2.12 秒，EtherDB 要 11.26 秒。**这一项 EtherDB 输了约 5.5 倍**，原因我在第七节里拆开讲（主要是客户端 API 语义差异），不找借口。
...
> ⚠️ **预告**：同一张表、同一条 SQL，TDengine 3.4.2.8 只用约 **2.06 秒**（2.1.7.2 老版本 2.12 秒）。这一项 EtherDB 输了约 5.5 倍。原因拆解见第七节——我不打算藏这个数字。
...
### 7.2 取数：两代 TDengine 都快 5 ~ 7 倍（这一项 EtherDB 输了）

| 取数方式（1002 万行 × 2 列） | EtherDB | **TDengine 3.4.2.8** | TDengine 2.1.7.2 |
|---|---|---|---|
| 逐行接口（`fetchRow` / `taos_fetch_row`） | 11.26 s（C++）/ 12.59 s（C） | **约 2.06 s**（6 次测量中位） | **2.12 s** |
| 批量接口（`etdb_fetch_block` / `taos_fetch_block`） | 11.55 s | **约 1.66 s**（2451 块） | **1.21 s**（3022 块） |
| 折算吞吐 | 约 80 ~ 89 万行/秒 | **约 486 万 / 603 万行/秒** | 474 万 / 826 万行/秒 |
...
2. **服务端每批「解码 + 序列化」路径**：EtherDB 的 FETCH 批按字节预算打包（本次约 130 KB/批、77 个往返），每次都要走一遍块解码与序列化；TDengine 的块协议按固定行块流水线推进（本机 3.4 为 2451 块，2.1.7.2 为 3022 块）。（注：EtherDB 服务端日志显示每 5.8 万行解码+序列化约 35 ms，服务端不是主要瓶颈。）

**一个值得注意的细节**：3.4 的块取数（约 1.66 s）反而比 2.1.7.2（1.21 s）慢约 37%，逐行取数基本持平（2.06 vs 2.12 s）。逐行/块各复测了 6 轮，结论稳定（逐行 2.02~2.42 s，块 1.58~1.83 s）。原因我们不做猜测，只把数字照实贴出——换环境复现出不同结果，欢迎来对数据。
...
| 全量取数（fetch all） | **TDengine** | ~5.5 ~ 7× | ~5 ~ 9× |
